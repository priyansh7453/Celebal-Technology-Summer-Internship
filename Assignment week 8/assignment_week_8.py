...
# 1. First save the current notebook
Javascript('IPython.notebook.save_notebook();')

# 2. Get the current notebook filename
notebook_name = !ls *.ipynb
current_notebook = notebook_name[0] if notebook_name else "Assignment Week_8.ipynb"

# 3. Clean metadata (optional but recommended)
!jupyter nbconvert --to notebook --ClearMetadataPreprocessor.enabled=True "{current_notebook}"

# 4. Verify file exists
if os.path.exists(current_notebook):
    print(f"Downloading notebook: {current_notebook}")
    files.download(current_notebook)
else:
    print("Error: Notebook file not found. Please save your notebook manually first.")
    print("Go to File > Save in the Colab menu, then try downloading again.")